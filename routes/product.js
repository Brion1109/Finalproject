...
router.get("/products",function(req,res){
	Product.find({},function(err,products){
		if(err){
			...
			res.render("products/index",{products:products,csrfToken: req.csrfToken()});
		}
	});
});
		...
		Product.find({name: regex}, function(err, allproducts){
		   if(err){
			  ...
			  if(allproducts.length < 1) {
				  ...
				  res.render("products/src",{noMatch: noMatch});
			  }
			  res.render("products/index",{products:allproducts, noMatch: noMatch,csrfToken: req.csrfToken()});
		   }
		});
	} else {
		// Get all  from DB
...
router.get("/products/new",isAdmin,function(req,res){
	res.render("products/new",{user: req.user,csrfToken: req.csrfToken()});
});
...
router.get("/products/:id",function(req,res){
	Product.findById(req.params.id).populate("reviews").exec(function(err,product){
		if(err || !product) {
			...
			res.render("products/show",{product: product,csrfToken: req.csrfToken()});
		}
	});
});
...
router.get("/products/:id/edit",isAdmin,function(req,res){
	Product.findById(req.params.id,function(err,product){
		if(err || !product) {
			req.flash("error","Product not found!!ww");
			res.redirect("/products");
		} else {
			res.render("products/edit",{product: product,csrfToken: req.csrfToken()});
		}
	});
});
