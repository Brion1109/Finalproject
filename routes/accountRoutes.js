...
router.get('/send-verification-email', checkAuth, async (req, res) => {
    ...
    if (req.user.isVerified || req.user.provider == 'google') {
        ...
        res.render('index/profile', { username: req.user.username, verified: req.user.isVerified, emailsent: true });
    }
});
...
router.get('/verifyemail', async (req, res) => {
    ...
    if (token) {
        ...
        if (check) {
            ...
            res.render('index/profile', { username: req.user.username, verified: req.user.isVerified, err: "Invalid token or Token has expired, Try again." });
        }
    } else {
        // doesnt have a token
...
router.get('/forgot-password', async (req, res) => {
    ...
    res.render('index/forgot-password', { csrfToken: req.csrfToken() });
...
router.post('/forgot-password', async (req, res) => {
    ...
    if (userData) {
        if (userData.provider == 'google') {
            ...
            res.render('index/forgot-password', { csrfToken: req.csrfToken(), msg: "User exists with Google account. Try resetting your google account password or logging using it.", type: 'danger' });
        } else {
            ...
            res.render('index/forgot-password', { csrfToken: req.csrfToken(), msg: "Reset email sent. Check your email for more info.", type: 'success' });
        }
    } else {
        res.render('index/forgot-password', { csrfToken: req.csrfToken(), msg: "No user Exists with this email.", type: 'danger' });
...
router.get('/reset-password', async (req, res) => {
    ...
    if (token) {
        ...
        if (check) {
            ...
            res.render('index/forgot-password', { csrfToken: req.csrfToken(), reset: true, email: check.email });
        } else {
            res.render('index/forgot-password', { csrfToken: req.csrfToken(), msg: "Token Tampered or Expired.", type: 'danger' });
        }
    } else {
        // doesnt have a token
        // I will simply redirect to profile 
...
router.post('/reset-password', async (req, res) => {
    ...
    if (!password || !password2 || (password2 != password)) {
        res.render('index/forgot-password', { csrfToken: req.csrfToken(), reset: true, err: "Passwords Don't Match !", email: email });
    } else {
        ...
        if (salt) {
            ...
            res.render('index/forgot-password', { csrfToken: req.csrfToken(), reset: true, err: "Unexpected Error Try Again", email: email });
